...
class Manageuserlist extends CI_Controller
{
	...
	function index($offset = 0, $order_column = 'Username', $order_type = 'asc')
	{
		...
		anchor('manageuserlist/index/'.$offset.'/Status/'.$new_order, 'Status'),
		...
		foreach ($Users as $Users) {
			...
			//date("F j, Y, g:i a")
			//date("Y-m-d H:i:s")
		anchor_popup('manageuserlist/update/'.$Users->PKID,'Update',array('class'=>'update'), $upd).' &nbsp &nbsp &nbsp &nbsp'. 
	...
	// run validation
		if ($this->form_validation->run() === FALSE){
			...
			if (($data['Role'] == "SuperAdmin")  || ($data['Role'] == "SuperAdmin")) { 
				$this->load->view('pages/manageusereditv2_view', $data);
				} else{
				$this->load->view('pages/unauth_view', $data);
				}
			...
			$id = $this->manageusermodel->create_user($Rolename, $Operator, $Username,$Employee_no, $Password, $Email, $Status);
