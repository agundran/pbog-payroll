...
class Payrollrunsmodel extends CI_Model{
	...
	function add_new_payrollruns($year, $month, $period, $payrollrun_type, $description, $transact_date,$execom_status )
	{
		$status = 'unposted';
		$new_data= array(
	...
	function get_processed_payroll_print($order_column){
	...
	function get_by_choice($choice){	
		$this->db->select('*')                        
		->where('employee_details.employee_no',$choice);
		return $this->db->get('employee_details');
	}


	function get_by_payroll_runs_details($id){	
		$this->db->select('*')                        
		->where('payroll_runs_details.payr_detail_id',$id);
		return $this->db->get('payroll_runs_details');
	}
	...
	function get_by_id($id){	
		$this->db->select('*')                        
		->where('payroll_runs.payr_id',$id);
		return $this->db->get('payroll_runs');
	}
   
	function get_by_employee($id){	
		$this->db->select('*')                        
	...
	function insert_data_for_payroll($choice){

		$limit = 1;
		$offset = 0;
		$query = $this->db->get_where('employee_details', array( 'employee_no' => $choice))->result_array();
      
		foreach ($query->result() as $row) {
			$this->db->insert('payroll_runs_details',$row);
	  }
	}
