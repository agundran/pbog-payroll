...
class philhealthtablemodel extends CI_Model{

	private $primary_key='salary_bracket';
	
	private $table_name='philhealth-table';
	...
	//else{
		$query = $this->db->select('*')
                        ->from('philhealth-table')
                        //->join('usersinroles', 'users.Username= usersinroles.Username')
						//->like('philhealthtable.salary_credit', $filter, 'after')
                        ->get('', $limit, $offset); 
	...
	$this->db->select('*')                        
						->where('philhealth-table.salary_bracket',$id);
	...
	function add_new_philhealthtable($tbl_id, $min_range, $max_range, $salary_credit, $ss_er, $ss_ee, $ss_total, $ec_er, $total_contri, $se_vm_ofw_total_contri, $effectivedate)
	
	{
		...
		$new_member_insert_data= array(
				...
				//$insert = 
				$this->db->insert('philhealthtable',$new_member_insert_data );
